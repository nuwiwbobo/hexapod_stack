...
//   2. RadianToTickPositive - M_PI/2 → ~768
//   3. RadianToTickNegative - -M_PI/2 → ~256
...
//
// Run: colcon build --packages-select hexapod_servo && colcon test --packages-select hexapod_servo
...
TEST_F(ServoDriverTest, RoundTrip) {
  ...
  EXPECT_NEAR(original, recovered, 0.01);  // Tolerance due to integer quantization
}

// TODO: Implement test - extreme values should be clamped to [0, ticks]
...
// TODO: Implement test - sign=1 vs sign=-1 should produce symmetric results around center
